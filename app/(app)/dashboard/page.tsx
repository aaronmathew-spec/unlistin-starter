...
export default function DashboardPage() {
  ...
  const metrics = useMemo(() => {
    ...
    if (!d) return null;
    return [
      { title: "Total Requests", primary: d.requests.total, sub: `${d.requests.open} open · ${d.requests.in_progress} in-progress`, href: "/requests" },
      { title: "Coverage Items", primary: d.coverage.total, sub: `${d.coverage.resolved} resolved`, href: "/coverage" },
      { title: "Brokers", primary: d.brokers.total, sub: "", href: "/brokers" },
    ];
  }, [data]);
        ...
        <section className="kpi-wrap" style={{ marginTop: 16 }}>
          {(metrics ?? []).map((m) =>
            loading ? (
              ...
              <Link key={m.title} href={m.href} className="panel" style={{ textDecoration: "none", color: "inherit" }}>
