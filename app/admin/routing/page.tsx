...
export default function RoutingPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const oOrgs = o.orgs ?? [];
      const rRoutes = r.routes ?? [];
      setOrgs(oOrgs);
      setRoutes(rRoutes);
      if (!orgId && oOrgs.length > 0) {
        setOrgId(oOrgs[0]!.id);
      }
    } catch (e: any) {
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      <div className="grid gap-2 md:grid-cols-[1fr,220px,120px]">
        <input
          ...
          placeholder="to address (e.g. bot@yourdomain.com)"
          value={toAddr}
        ...
        <button
          onClick={onCreate}
          disabled={busy || !toAddr.trim() || !orgId}
        ...
        {routes.map((r) => (
          <div key={r.id} className="border rounded p-3 flex items-center justify-between">
            <div className="text-sm">
              ...
              <div>
                <b>org:</b> {r.org_name} <span className="text-gray-500">({r.org_id})</span>
              </div>
            </div>
