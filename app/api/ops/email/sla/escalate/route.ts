// app/api/ops/email/sla/escalate/route.ts
// Secure, explicit escalation sender (non-breaking).
import { NextResponse } from "next/server";
import { buildEscalationEmail } from "@/src/lib/email/templates/followups";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const ccRegulator = body.ccRegulator ? String(body.ccRegulator) : undefined;

    const { subject, body: text } = buildEscalationEmail({
      controllerName,
      region,
      subjectFullName,
      subjectEmail,
    ...
    const res = await sendEmail({
      ...
      subject,
      text,
      cc: ccRegulator ? [ccRegulator] : undefined,
    });
