// src/lib/email/templates/controllers/timesjobs.ts
...
export type TimesJobsEmailArgs = {
...
export function buildTimesJobsRemovalEmail(args: TimesJobsEmailArgs): { subject: string; body: string } {
  const policy = resolvePolicyByRegion(args.region);
  const subject = `Data Erasure Request — TimesJobs`;
  const who = args.subjectFullName ? `${args.subjectFullName}` : "the data subject";

  const body = lineBreaks(
    `Hello TimesJobs Privacy Team,`,
    "",
