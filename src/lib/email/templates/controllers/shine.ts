// src/lib/email/templates/controllers/shine.ts
...
export function buildShineRemovalEmail(args: ShineEmailArgs): { subject: string; body: string } {
  const policy = resolvePolicyByRegion(args.region);
  const subject = `Data Erasure Request — Shine.com`;
  const who = args.subjectFullName ? `${args.subjectFullName}` : "the data subject";

  const body = lineBreaks(
    `Hello Shine Privacy Team,`,
    "",
