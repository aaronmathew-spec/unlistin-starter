...
export async function requirePAT(req: Request) {
  ...
  const { data, error } = await db.from("api_keys").select("id,user_id,hash,revoked_at,scopes").eq("prefix", prefix).limit(1).single();
  ...
  db.from("api_keys").update({ last_used_at: new Date().toISOString() }).eq("id", data.id).then().catch();

  return { ok: true as const, userId: data.user_id as string, scopes: (data.scopes || []) as string[] };
}
