// lib/scan/brokers/timesjobs.ts
/**
 * TimesJobs adapter (server-only).
 */
...
export async function queryTimesJobs(input: Partial<ScanInput>): Promise<RawHit[]> {
  const name = (input.name ?? "").trim();
  const city = (input.city ?? "").trim();
  const email = (input.email ?? "").trim();

  const q = encodeURIComponent([name, city].filter(Boolean).join(" ") || email.split("@")[0] || "");
  const url = `https://www.timesjobs.com/candidate/job-search.html?searchType=personalizedSearch&from=submit&txtKeywords=${q}`;
