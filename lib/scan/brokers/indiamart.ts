// lib/scan/brokers/indiamart.ts
/**
 * IndiaMART adapter (server-only, dependency-free).
 * Emits your RawHit shape and falls back safely if blocked.
 */
...
export async function queryIndiaMart(input: Partial<ScanInput>): Promise<RawHit[]> {
  ...
  const url = `https://www.indiamart.com/search.mp?ss=${q}`;
  ...
  const fallback: RawHit = {
    ...
    fields: {
      ...
      snippet:
        (name && city)
          ? `${name} possibly listed in ${city} on IndiaMART.`
          : `Possible listing on IndiaMART.`,
    },
  };
  ...
  while ((m = titleRe.exec(html)) && seen < 5) {
    ...
    let link: string;
    try {
      const u = new URL(href, "https://www.indiamart.com");
      link = u.toString();
